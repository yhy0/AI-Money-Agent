...
2. **杠杆选择 (经优化)**:
   ...
   - 高信念度 (0.7-1.0): 使用 8-15x 杠杆 (最高15x以控制风险)
3. **多样化**: 避免将超过 40% 的资本集中在单一头寸
4. **费用影响**: 由于总资金极小，任何一笔交易的固定费用都可能对利润产生巨大影响。请避免建立价值低于 $5 的头寸，因为它们很可能无利可图
5. **清算风险 (重要)**: 你的清算价格由杠杆决定。**清算距离 ≈ 1 / 杠杆率**。例如，10x杠杆意味着约10%的反向价格变动就会导致清算。**你必须确保你的止损价格 (`stop_loss_price`) 在清算价格之内**，为市场波动留出足够缓冲。
6. **⚠️ 最小交易数量约束** (关键):
...
2. **stop_loss_price** (浮点数): 止损的确切价格水平
   - **单笔风险敞口**: 每笔交易的计划亏损 (由止损决定) 应限制在账户价值的 **3-8%**。在市场波动剧烈时可放宽至12%，但需有充分理由。
   - **动态止损**: 止损位置应参考ATR指标。高ATR意味着高波动，应设置更宽的止损；低ATR则可以设置更紧的止损。
   - 设置在近期支撑/阻力位之外，以避免过早止损。

3. **invalidation_condition** (字符串): 使你的交易理论无效的特定、可量化的市场信号。
   - **提供结构化示例**:
     - **技术指标类**: "3min MACD死叉且跌破零轴", "4h RSI跌破50中性区"
     - **价格行为类**: "价格收盘跌破4h EMA50均线", "未能突破前高$105,000并形成双顶结构"
     - **市场情绪类**: "BTC资金费率在4小时内由正转负", "BTC 跌破 $100k"
   - 必须是客观且可观察的
...
- **高波动趋势 (High-Volatility Trend)**:
  - 趋势明显，价格变动快。
  - **策略**: 顺势而为，可使用标准仓位和杠杆，但需设置较宽的止损（参考ATR）以避免被震荡出局。
- **低波动盘整 (Low-Volatility Consolidation)**:
  - 价格在窄幅区间内波动，方向不明。
  - **策略**: 降低交易频率，减小仓位和杠杆。止损可以设得更紧。或选择 `hold` 等待突破。
...
**数据说明**：
- 以上数据包含所有配置的交易币种的完整技术分析
- 每个币种都可以交易，请综合分析选择最佳机会
- 建议优先选择趋势明确、信号强烈的币种
